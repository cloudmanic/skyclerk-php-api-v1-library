...
Document Url: http://skyclerk.com/api-v1/contacts/update-contact-entry
	
Description: Example of making a call to the API to insert a 
							contact entry, updating, get by id, and delete a contact entry 
*/
...
// Update the contact
$SK->set_data("ContactsName", 'Acme Computers Inc.');
$SK->set_data("ContactsType", "Both"); // Vendor, Customer, Both

if($data = $SK->update_contact_by_id($id)) {
  echo "OK - Update Contact passed. Contact Id: $id<br />";	
}	else {
  echo "FAILED - Update Contact failed with message: " . $SK->error_to_string() . "<br />";
}
